package com.example.tusm.newluanch.dp;
...
public class DBManager {
    ...
    // 添加到数据库
    public void addToDB(String title, String type,String content, String time) {
        //  组装数据
        ContentValues cv = new ContentValues();
        cv.put(NoteDBOpenHelper.TYPE, type);
    ...
    public void readFromDB(List<Note> noteList) {
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                note.setType(cursor.getString(cursor.getColumnIndex(NoteDBOpenHelper.TYPE)));
    ...
    //  更新数据
    public void updateNote(int noteID,String type, String title, String content, String time) {
        ContentValues cv = new ContentValues();
    ...
    // 根据type查询数据
    public List<Note> readTypeData(String  type) {
        List<Note> list = new ArrayList<Note>();
        Cursor cursor = dbReader.rawQuery("SELECT * FROM note WHERE type = ?", new String[]{type});
